        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto p-5">
            {items.length === 0 ? (
              ...
              <article key={item.id} className="flex gap-4 border-b border-navy/10 pb-5">
                <ProductImage src={item.imageUrl} sizes="96px" alt={item.name} className="h-32 w-24 rounded-xl bg-sand object-cover" />
                <div className="flex min-w-0 flex-1 flex-col"><div className="flex items-start justify-between gap-2"><div><p className="text-lg font-black">{item.name}</p><p className="mt-1 text-xs font-semibold leading-5 text-navy/45">{item.selections.map((selection) => `${selection.name} · ${selection.size}`).join(" / ")}</p></div><button type="button" onClick={() => removeItem(item.id)} className="focus-ring rounded-full p-2 text-navy/35 hover:text-terracotta" aria-label={`Retirer ${item.name}`}><Trash2 className="h-4 w-4" /></button></div>
                  <div className="mt-auto flex items-center justify-between">{item.kind === "individual" ? <div className="flex items-center rounded-full border border-navy/10"><button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)} className="focus-ring p-2.5" aria-label="Diminuer la quantité"><Minus className="h-3.5 w-3.5" /></button><span className="min-w-7 text-center text-xs font-bold">{item.quantity}</span><button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="focus-ring p-2.5" aria-label="Augmenter la quantité"><Plus className="h-3.5 w-3.5" /></button></div> : <span className="text-xs font-bold uppercase tracking-wider text-sea">1 pack</span>}<p className="font-bold">{formatCurrency(item.unitPrice * item.quantity)}</p></div>
                </div>
              </article>
            ))}
          </div>
