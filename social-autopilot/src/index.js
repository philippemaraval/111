...
function socialPostPreview(network, proposal, text, format) {
  const media = panelMediaMarkup(network, proposal, format);
  const avatar = previewLogo();
  if (network === "x") return `<div class="preview-post x-post"><div class="preview-account">${avatar}<span><strong>111 Marseille ✓</strong><small>@sunmedia111 · 1 min</small></span><b>•••</b></div><p>${escapeHtml(text).replace(/\n/g, "<br>")}</p>${media}<small class="post-time">12:23 · 14 sept. 2026 · 892 vues</small><div class="network-actions">♡ 12　♢ 4　♡ 38　▥ 892　↗</div></div>`;
  if (network === "tiktok") return `<div class="preview-post tiktok-post"><div class="video-tabs">Abonnements　<b>Pour toi</b></div>${media}<aside class="tiktok-actions">${avatar}<b>♥</b><small>365</small><b>●</b><small>105</small><b>↗</b><small>Partager</small></aside><div class="video-caption"><strong>@sunmedia.111</strong><p>${escapeHtml(text)}</p><small>♫ Son original · 111 Marseille</small></div><div class="phone-nav">⌂　⌕　⊕　▣　♙</div></div>`;
  return `<div class="preview-post instagram-post"><div class="story-progress"><i></i><i></i><i></i></div><div class="preview-account">${avatar}<span><strong>sunmedia.111 ✓</strong><small>Marseille · 1 min</small></span><b>•••</b></div>${media}<span class="carousel-count">1/${Math.max(2, platformMedia(proposal, network).length)}</span><aside class="reel-actions"><b>♡</b><b>◯</b><b>➤</b><b>•••</b></aside><div class="network-actions">♡　◯　➤　　　　　　　　　▢</div><small class="likes">Aimé par la communauté 111</small><p><strong>sunmedia.111</strong> ${escapeHtml(text).replace(/\n/g, "<br>")}</p><div class="story-reply">Envoyer un message　♡　➤</div><div class="phone-nav">⌂　⌕　⊕　▣　◎</div></div>`;
}

function previewLogo() {
  return `<span class="avatar preview-logo"><svg viewBox="0 0 192 192" aria-hidden="true"><path fill="#0092cd" d="M104 0C86 1 65 5 50 12 33 20 20 33 12 47 4 62 1 74 1 83c1 11 1 22 2 33 1 17 6 30 14 37 7 8 11 14 19 21 11 9 26 12 42 15 18 4 38 2 52-1 14-3 28-10 37-23 11-12 17-24 21-39 3-14 2-31 2-48 0-14-3-25-9-38-6-13-16-22-30-29-12-6-28-10-34-11Z"/><path fill="#fff" d="m37 83 25-41h7l-2 108-18-4 1-52-13-11Zm43 0 25-41h7l-2 108-18-4 1-52-13-11Zm44 0 24-41h7l-2 108-18-4 1-52-12-11Z"/></svg></span>`;
}
...
function formatStyles() {
  return formatStylesBase() + `.proposal>.previews{grid-column:1/-1;width:100%;margin-top:4px}.proposal>.previews .network-preview{height:100%}.preview-logo{padding:0;background:transparent;overflow:hidden}.preview-logo svg{display:block;width:100%;height:100%}.preview-account b{letter-spacing:2px}.post-time{display:block;color:#536471;padding:7px 0;border-bottom:1px solid #eff3f4}.x-post>p{font-size:.92rem;line-height:1.35;margin:8px 0}.x-post .panel-media{margin:8px 0;border:1px solid #cfd9de;border-radius:14px;overflow:hidden}.x-post .panel-media img,.x-post .panel-media video{border-radius:0}.instagram-post .likes{display:block;margin-top:8px;font-weight:700}.instagram-post>p{font-size:.78rem}.story-reply,.phone-nav,.video-tabs,.video-caption,.reel-actions{display:none}.network-instagram.format-story .instagram-post>p,.network-instagram.format-reel .instagram-post>p{display:block;position:absolute;z-index:3;left:12px;right:52px;bottom:53px;margin:0;color:#fff;text-shadow:0 1px 5px #000}.network-instagram.format-story .likes,.network-instagram.format-reel .likes{display:none}.network-instagram.format-story .story-reply{display:block;position:absolute;z-index:4;left:10px;right:10px;bottom:10px;border:1px solid #fff9;border-radius:999px;padding:7px;color:#fff}.network-instagram.format-reel .reel-actions{display:grid;position:absolute;z-index:4;right:9px;bottom:55px;gap:15px;color:#fff;font-size:1.1rem}.network-instagram.format-reel .phone-nav{display:block;position:absolute;z-index:4;left:0;right:0;bottom:0;padding:9px;background:#111d;text-align:center;color:#fff}.tiktok-post{max-width:245px;margin:12px auto 0;aspect-ratio:9/16;background:#111;color:#fff}.tiktok-post .panel-media{position:absolute;inset:0;margin:0}.tiktok-post .panel-media img,.tiktok-post .panel-media video{height:100%;aspect-ratio:auto}.tiktok-post .video-tabs{display:block;position:absolute;z-index:3;top:12px;left:0;right:0;text-align:center;text-shadow:0 1px 4px #000}.tiktok-post .video-caption{display:block;position:absolute;z-index:3;left:10px;right:45px;bottom:42px;text-shadow:0 1px 4px #000}.tiktok-post .video-caption p{margin:4px 0;font-size:.72rem}.tiktok-post .phone-nav{display:block;position:absolute;z-index:4;left:0;right:0;bottom:0;padding:9px;background:#111d;text-align:center}.network-tiktok .tiktok-actions{display:grid}.tiktok-post>.text-only-preview{display:none}@media(max-width:980px){.proposal>.previews .network-preview{height:auto}}`;
}

function formatStylesBase() {
  return `.platform:has(input[data-network=instagram]:checked){color:#fff;border-color:#d62976;background:linear-gradient(135deg,#feda75,#d62976,#4f5bd5)}.platform:has(input[data-network=tiktok]:checked){color:#fff;border-color:#25f4ee;background:#111;box-shadow:-2px 2px #25f4ee,2px -2px #fe2c55}.platform:has(input[data-network=x]:checked){color:#fff;border-color:#111;background:#111}.platform:has(input:checked) input{accent-color:#fff}.previews{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-items:start;gap:12px;background:transparent;padding:0}.preview-tabs{display:none}.network-preview,.network-preview.active{display:block;min-width:0;border:2px solid;padding:12px}.network-instagram{background:#fff7fb;border-color:#d62976}.network-tiktok{background:#f3ffff;border-color:#25f4ee;box-shadow:inset -3px 0 #fe2c55}.network-x{background:#f5f5f5;border-color:#111}.network-preview[hidden]{display:none!important}.panel-title{display:flex;align-items:center;gap:8px;margin-bottom:10px}.panel-title h3{margin:0;font-size:1rem}.network-mark{display:grid;place-items:center;width:30px;height:30px;border-radius:9px;background:#111;color:#fff;font-size:1.1rem}.network-instagram .network-mark{background:linear-gradient(135deg,#feda75,#d62976,#4f5bd5)}.network-tiktok .network-mark{box-shadow:-2px 2px #25f4ee,2px -2px #fe2c55}.format-card{background:#ffffffb8;border-radius:12px;padding:10px;margin:0 0 10px}.format-card label{margin-top:8px!important;font-size:.7rem}.format-card input[type=file]{background:#fff;padding:8px;font-size:.72rem}.preview-post{position:relative;margin-top:12px;background:#fff;padding:10px;border:1px solid #d8dadd;border-radius:12px;overflow:hidden;font-size:.82rem}.network-x .preview-post{border-radius:0;border-color:#cfd9de}.preview-account{position:relative;z-index:3;justify-content:space-between}.preview-account>span:nth-child(2){flex:1}.preview-account small{display:block;float:none;color:#687684;font-size:.68rem}.panel-media{display:grid;grid-template-columns:1fr;gap:3px;margin:8px -10px}.panel-media img,.panel-media video{display:block;width:100%;aspect-ratio:1;max-height:520px;object-fit:cover}.carousel-media{grid-template-columns:repeat(2,minmax(0,1fr))}.carousel-count,.story-progress,.tiktok-actions,.story-caption{display:none}.format-carousel .carousel-count{display:block;position:absolute;right:16px;top:64px;background:#111c;color:#fff;padding:4px 7px;border-radius:999px}.network-instagram.format-story .preview-post,.network-instagram.format-reel .preview-post,.network-tiktok.format-video .preview-post{max-width:245px;margin:12px auto 0;aspect-ratio:9/16;background:#111;color:#fff;padding:10px}.network-instagram.format-story .panel-media,.network-instagram.format-reel .panel-media,.network-tiktok.format-video .panel-media{position:absolute;inset:0;margin:0;z-index:0}.network-instagram.format-story .panel-media img,.network-instagram.format-story .panel-media video,.network-instagram.format-reel .panel-media img,.network-instagram.format-reel .panel-media video,.network-tiktok.format-video .panel-media img,.network-tiktok.format-video .panel-media video{height:100%;aspect-ratio:auto}.network-instagram.format-story .preview-account,.network-instagram.format-reel .preview-account,.network-tiktok.format-video .preview-account{color:#fff;text-shadow:0 1px 5px #000}.network-instagram.format-story .preview-account small,.network-instagram.format-reel .preview-account small,.network-tiktok.format-video .preview-account small{color:#fff}.network-instagram.format-story .story-progress{display:flex;position:relative;z-index:3;gap:3px;margin-bottom:8px}.story-progress i{height:2px;flex:1;background:#fff9;border-radius:2px}.network-instagram.format-story .story-caption,.network-instagram.format-reel .story-caption{display:grid;position:absolute;z-index:3;left:12px;right:12px;bottom:52px;text-shadow:0 1px 5px #000}.network-instagram.format-story .preview-post>p,.network-instagram.format-reel .preview-post>p,.network-tiktok.format-video .preview-post>p{display:none}.network-tiktok.format-video .tiktok-actions{display:grid;position:absolute;z-index:3;right:8px;bottom:55px;text-align:center;gap:3px}.tiktok-actions b{font-size:1.25rem}.tiktok-actions small{font-size:.55rem}.network-instagram.format-story .network-actions,.network-instagram.format-reel .network-actions,.network-tiktok.format-video .network-actions{position:absolute;z-index:3;left:10px;right:10px;bottom:10px;border:0;color:#fff;text-shadow:0 1px 5px #000}.format-text .panel-media{display:none}.format-text .text-only-preview{display:none}.format-badge{position:absolute;z-index:4;right:10px;top:10px;background:#111b;color:#fff;border-radius:999px;padding:3px 7px;font-size:.58rem;text-transform:uppercase}.text-only-preview{padding:18px;background:#f5f2eb;border-radius:12px;color:#12202f99;text-align:center}.network-actions{padding-top:9px;border-top:1px solid #12202f14;font-size:.68rem;font-weight:700;white-space:nowrap}.generation-style{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr 2fr;gap:12px;background:#ffffff12;border-radius:14px;padding:14px}.generation-style legend{color:#ffd43b}.hashtag-options{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.hashtag-options>span{width:100%;font-weight:800}.hashtag-options label{display:flex;align-items:center;gap:5px}.rejected-controls{display:grid;gap:8px;margin:10px 0}.rejected-controls form{display:flex;align-items:center;justify-content:space-between;gap:12px;background:#fff;padding:10px 14px;border-radius:14px}.rejected-controls button{margin:0}@media(max-width:980px){.previews{grid-template-columns:1fr}.network-preview{padding:16px}}@media(max-width:700px){.generation-style{grid-template-columns:1fr}.rejected-controls form{align-items:flex-start;flex-direction:column}.carousel-media{grid-template-columns:1fr}}`;
}

function mediaFormScripts() {
  return `document.querySelectorAll('.edit-form').forEach(form=>{form.enctype='multipart/form-data';form.querySelectorAll('[data-network]').forEach(box=>{const panel=form.querySelector('[data-preview="'+box.dataset.network+'"]');const update=()=>panel.hidden=!box.checked;box.addEventListener('change',update);update()});form.querySelectorAll('[data-format]').forEach(select=>{const panel=select.closest('[data-preview]');const update=()=>{[...panel.classList].filter(x=>x.startsWith('format-')).forEach(x=>panel.classList.remove(x));panel.classList.add('format-'+select.value);const empty=panel.querySelector('.text-only-preview');if(empty)empty.hidden=select.value!=='text';const badge=panel.querySelector('.format-badge');if(badge)badge.textContent=select.options[select.selectedIndex].text};select.addEventListener('change',update);update()});form.querySelectorAll('input[type=file][name$=_media]').forEach(input=>input.addEventListener('change',()=>{const holder=input.closest('[data-preview]').querySelector('.panel-media');if(!holder||!input.files.length)return;holder.innerHTML=[...input.files].map(file=>file.type.startsWith('video/')?'<video controls src="'+URL.createObjectURL(file)+'"></video>':'<img src="'+URL.createObjectURL(file)+'" alt="">').join('')}));queueMicrotask(()=>{const previews=form.querySelector('.previews');if(!previews)return;previews.querySelectorAll('input,select,textarea').forEach(control=>control.setAttribute('form',form.id));form.closest('.proposal').append(previews)})});`;
}
