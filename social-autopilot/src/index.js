...
function networkPreview(network, p, active = false) {
  ...
  return `<section class="network-preview network-${network} format-${selectedFormat} ${active ? "active" : ""}" data-preview="${network}" ${enabled ? "" : "hidden"}><div class="panel-title"><span class="network-mark">${network === "instagram" ? "◎" : network === "tiktok" ? "♪" : "𝕏"}</span><h3>${label}</h3></div>${formatEditor(network, selectedFormat)}<label>Texte ${label}<small><span data-count="${network}">${String(text).length}</span>/${limit}</small><textarea name="${network === "x" ? "x_text" : `${network}_text`}" data-text="${network}" rows="${rows}" maxlength="${limit}">${escapeHtml(text)}</textarea></label><div class="preview-post"><div class="story-progress"><i></i><i></i><i></i></div><div class="preview-account"><span class="avatar">111</span><span><strong>sunmedia.111</strong><small>${network === "x" ? "@sunmedia111 · 1 min" : network === "tiktok" ? "@sunmedia.111 · Suivre" : "Marseille · 1 min"}</small></span><b>•••</b></div><div class="format-badge">${selectedFormat}</div>${panelMediaMarkup(network, p, selectedFormat)}<span class="carousel-count">1/${Math.max(2, platformMedia(p, network).length)}</span><aside class="tiktok-actions"><b>♥</b><small>J’aime</small><b>●</b><small>Commentaires</small><b>↗</b><small>Partager</small></aside><div class="story-caption"><strong>sunmedia.111</strong><span>${escapeHtml(text)}</span></div><p>${escapeHtml(text).replace(/\n/g, "<br>")}</p><div class="network-actions">${network === "instagram" ? "♡　⌁　➤　　　　　　　▢" : network === "tiktok" ? "♫ Son original · sunmedia.111" : "♡　12　　♢　4　　♡　38　　▥　892　　↗"}</div></div><details class="platform-kit"><summary>Options avancées</summary>${advanced}</details></section>`;
}
...
function formatStyles() {
  return formatStylesBase() + `.proposal>.previews{grid-column:1/-1;width:100%;margin-top:4px}.proposal>.previews .network-preview{height:100%}`;
}
